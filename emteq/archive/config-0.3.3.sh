...
#and is written specifically for pilbal units. 
...
res=0
ver="0.3.4"
dbdir="/home/emteq/.resources/db/"
dbstatic="/home/emteq/.resources/dbstatic/"
dir="/home/emteq/.resources/"
confdir="/home/emteq/.resources/config/"
c2=$( cat /home/emteq/.c2 )
#c2trim=${#c2}
#c2trim=$((c2trim-4))
#basec2=$( echo $c2 | trim -c 1-$c2trim)
...
#Count number of configuration files that require transfer
...
#Loads selected Map to Flash Drive. 
function makekey {
 header
 echo " "
...
#Use MUTT to email configuration files to EMTEQ
function sendconfig {
  cnt=1
  header
...
echo  "1. eConnect IP: "		$econip
echo "2. Base GUI File: "	$c2 
echo "3. Database Files:"
echo "	CWR450-2000: " ${dbfiles[1]}
echo "	CWR450-5000: " ${dbfiles[2]}
echo "4. Update System with EMTEQ package: "
echo " "
while : ; do
  ...
  case $junk in
    ...
    2) echo " "
       ...
       break;;
    3) echo "Warning - Selecting the wrong file will cause configurations to fail!"
       echo " "
       cnt=1
...
#Dumbs local DB to file for import to remote system
function fms_config () {
count=301
cnt=1
...
case $type in
  1)  mysql -uroot -proot < $dbdir${dbfiles[1]};;
  2)  mysql -uroot -proot < $dbdir${dbfiles[2]};;
  3)  mysql -uroot -proot < $dbdir${dbfiles[1]};;
  4)  mysql -uroot -proot < $dbdir${dbfiles[2]};;
esac
  ...
  while : ; do
    ...
    case $type in 
      [1-4]) echo -n "Enter Lot Number: "
             ...
             read tail
             echo -n "AVOIP enabled, (y)es (n)o: " #***Need to determine how to set RUNMODE 5***
             read sip
             while : ; do
               case $sip in
                 y) break;;
...
#burn image to flash disk 
  echo "Transfering image to USB flash disk. There may be a slight delay at 100% to allow for sync."
...
#cp $dir"GUI/"$c2".tgz" $dir"payload/temp"
...
echo ""
echo -e "${red}Configuration Complete, Rebooting eConnect${white}"
echo "The initial reboot may take up to 10 minutes to complete"
echo " "
echo -ne "${red}Press any key to continue${white}"
read junk
header
...
while : ; do
  ...
  echo "2. Generate 2.1.10 USB Flash Drive"
  ...
  if [ $config_file_cnt -gt 5 ] ; then
    ...
    echo "Please ensure system has active internet connection and select option 4."
  else
