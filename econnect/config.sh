...
lopa_asset="/home/econnect/csrc/presentation/themes/pc24/public/img/lopa/"
...
function usbkeysel() {
cnt=0
for list in /run/media/econnect/* ; do
  if [ -d "$list" ]; then
    cnt=$((cnt + 1))
...
function sshraw() {
  sshpass -f /home/econnect/.id ssh emteq@$econip '$1'
}
...
function sshbr() { 
sshpass -f /home/econnect/.id ssh emteq@$econip bash -c "$1" 
}
...
while : ; do
  ...
  case $junk in
    ...
    13) header
      ...
      echo " "
      count=1
      for i in $( ls /run/media/econnect/${usbkey[$usbselect]}/*.tgz ) ; do
        echo $count"." ${i##*/}
        updatefiles[$count]=$i      
        count=$(($count+1))    
      done
        ...
        case $updatefile in    
          [0-9]|[0-9][0-9])
            if [ $updatefile -lt $count ] ; then
              rm -rf /home/econnect/.resources/db/*
              rm -rf /home/econnect/.resources/GUI/*
...
case $type in
[1-4]) 
  tar -xzpf ${dir}GUI/$c2_450  -C /home/econnect &> /dev/null
  cp ${dir}json/resource.json /home/econnect/$resfile
  tar -czpf ${dir}payload/temp/$c2_450 c2/
  echo "$c2_450" > ${dir}payload/update_LIST_${lot}_Factory.lst
  ;;
[5-8])
 tar -xzpf ${dir}GUI/$c2_451  -C /home/econnect &> /dev/null
 cp ${dir}json/resource.json /home/econnect/$resfile
...
badpart=0
map_partsize=$(sshpass -f /home/econnect/.id ssh emteq@$econip 'df /mnt/mmap | tail -n 1')
map_partsize=$(echo $map_partsize | cut -d ' ' -f 2)
if [[ $map = 6 ]] && [[ $map_partsize < 160000000 ]]; then 
  warning
  badpart=1
elif [[ $map_partsize < 140000000 ]]; then
  warning
  badpart=1
fi
sleep 20
if [[ $badpart = 0 ]]; then   #not elegant, but gets it done. 
  if [ $maptrans = 1 ] && [ "$map" != "7" ]; then
    ...
    case $map in 
      1) sshpass -f /home/econnect/.id rsync --progress "$dir"map/na.mbtiles emteq@$econip:/mnt/mmap/map.mbtiles;;
      2) sshpass -f /home/econnect/.id rsync --progress "$dir"map/eu.mbtiles emteq@$econip:/mnt/mmap/map.mbtiles;;
      3) sshpass -f /home/econnect/.id rsync --progress "$dir"map/me.mbtiles emteq@$econip:/mnt/mmap/map.mbtiles;;
      4) sshpass -f /home/econnect/.id rsync --progress "$dir"map/sa.mbtiles emteq@$econip:/mnt/mmap/map.mbtiles;;
      5) sshpass -f /home/econnect/.id rsync --progress "$dir"map/as.mbtiles emteq@$econip:/mnt/mmap/map.mbtiles;;
      6) sshpass -f /home/econnect/.id rsync --progress "$dir"map/wo.mbtiles emteq@$econip:/mnt/mmap/map.mbtiles;;
    esac
    echo " "
  ...
  #deliver payload
  sshpass -f /home/econnect/.id scp $dir"payload/update_LIST_"$lot"_Factory.tgz" emteq@$econip:/mnt/user/upload/
  sshpass -f /home/econnect/.id scp $dir"payload/update_LIST_"$lot"_Factory.lst" emteq@$econip:/mnt/user/upload/
  sshpass -f /home/econnect/.id scp $confdir$lot emteq@$econip:/mnt/user/upload/.config_$lot.txt 
  #placeholder for for slideshow widget
  sshpass -f /home/econnect/.id ssh emteq@$econip bash -c "'mkdir /mnt/user/upload/slideshow'"
  sshpass -f /home/econnect/.id scp ${dir}other/greyframe.png emteq@$econip:/mnt/user/upload/slideshow/
  sshpass -f /home/econnect/.id scp ${dir}other/README.TXT emteq@$econip:/mnt/user/upload/slideshow/
  sshpass -f /home/econnect/.id scp ${dir}other/presentation_time.txt emteq@$econip:/mnt/user/upload/slideshow/
  ...
  sshpass -f /home/econnect/.id scp $dir"other/update" emteq@$econip:/mnt/user/upload/
  #clean up
  rm $dbstatic'update_DB_factory.sql'
  rm $dir'payload/temp/'*
  rm $dir'payload/'*
...
result=$(adb install $dir/encoder/$encoder | head -c1)
echo $encoder
if [ $? = 0 ]; then
  echo " "
...
read n
while : ; do
	sshpass -f /home/econnect/.id ssh -t emteq@$econip "su -c 'pkill -9 econapp'"
	sshpass -f /home/econnect/.id ssh -t emteq@$econip "su -c /root/qualtests"
	clear
	header
...
function usb_check { 
	...
	while [ -z "$usb_flash" ];do
		usb_flash=$(sshpass -f /home/econnect/.id ssh -t emteq@$econip '/usr/sbin/lsusb | grep 058f:6387')
		sleep 1
		count=$((count+1))
		if [[ $count = 15 ]]; then 
			count=0
...
header
	 ...
	 echo ""
         ...
         if [[ $break_loop != 1 ]]; then
           ...
           isup
	   safe=$(sshpass -f /home/econnect/.id ssh emteq@$econip 'if [ -e /opt/scenes/autoupdate.sh ]; then echo 1; else echo 0; fi')
	   if [ $safe=0 ]; then
             echo -e "Please enter ${green}root${white} when prompted for password. Prompt will not echo"
             case $avoip in
               1) sshpass -f /home/econnect/.id ssh -t emteq@$econip 'su -c "/opt/eConnect/scripts/mode_swap/set_boot_mode.sh 1"';;
               2) sshpass -f /home/econnect/.id ssh -t emteq@$econip 'su -c "/opt/eConnect/scripts/mode_swap/set_boot_mode.sh 5"';;
               3) sshpass -f /home/econnect/.id ssh -t emteq@$econip 'su -c "/opt/eConnect/scripts/mode_swap/set_boot_mode.sh 2"';;
     	     esac
             echo " "
